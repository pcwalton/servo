...
impl ImageCache {
                    ...
                    Err(_) => {
                        match self.placeholder_image.clone() {
                            Some(placeholder_image) => {
                                self.complete_load(msg.url, LoadResult::PlaceholderLoaded(
                                        placeholder_image))
                            }
                            None => self.complete_load(msg.url, LoadResult::None),
                        }
                    }
                }
            }
    ...
    fn complete_load(&mut self, url: Url, mut load_result: LoadResult) {
