...
impl<Window: WindowMethods> IOCompositor<Window> {
            ...
            (Msg::DelayedCompositionTimeout(timestamp), ShutdownState::NotShuttingDown) => {
                if let CompositionRequest::DelayedComposite(this_timestamp) =
                    ...
                    if timestamp == this_timestamp {
                        //println!("delayed composition timeout!");
        ...
        match self.webrender_api {
            Some(ref webrender_api) => {
                ...
                for scroll_event in self.pending_scroll_zoom_events.drain(..) {
                    ...
                    if let Some(combined_event) = last_combined_event {
                        if combined_event.phase != scroll_event.phase {
                            ...
                            /*println!("scrolling: delta={:?} cursor={:?} phase={:?}",
                                     delta,
                                     cursor,
                ...
                if let Some(combined_event) = last_combined_event {
                    ...
                    /*println!("scrolling: delta={:?} cursor={:?} phase={:?} event_count={:?}",
                             delta,
                             cursor,
                             combined_event.phase,
    ...
    fn composite_specific_target(&mut self,
        ...
        //println!("... compositing now @ {:?}ms", (precise_time_ns() as f64) / 1000000.0);
        /*let these_pixels = gl::read_pixels(0,
                                           0,
                                           self.window_size.to_untyped().width as i32,
                                           self.window_size.to_untyped().height as i32,
                                           gl::BGRA,
                                           gl::UNSIGNED_BYTE);
        if these_pixels == self.last_pixels {
            println!("*** Composited an identical frame to the last one!");
        }
        self.last_pixels = these_pixels;*/

        // Perform the page flip. This will likely block for a while.
        self.window.present();

        self.last_composite_time = precise_time_ns();
        //println!("... composited @ {:?}ms", (self.last_composite_time as f64) / 1000000.0);
    ...
    pub fn handle_events(&mut self, messages: Vec<WindowEvent>) -> bool {
        ...
        match self.composition_request {
            ...
            CompositionRequest::CompositeNow(reason) => {
                /*println!("compositing due to {:?} @ {:?}ms...",
                         reason,
                         (precise_time_ns() as f64) / 1000000.0);*/
                self.composite()
            }
        }
