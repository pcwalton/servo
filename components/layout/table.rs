            ...
            if kid.is_table_colgroup() {
                ...
                match self.table_layout {
                    FixedLayout => {
                        ...
                        if !did_first_row {
                            did_first_row = true;
                            for (parent_column_inline_size, child_column_inline_size) in
                                    self.column_inline_sizes
                                        .mut_iter()
                                        .zip(child_column_inline_sizes.iter()) {
                                // FIXME(pcwalton): Is this correct?
                                if parent_column_inline_size.minimum_length == Au(0) {
                                    *parent_column_inline_size = *child_column_inline_size
                                }
                            }
                        }
                        ...
                        for i in range(parent_column_count, child_column_count) {
                            self.column_inline_sizes.push(*child_column_inline_sizes.get(i));
                        }
                    }
                    AutoLayout => {
                        let child_column_inline_sizes = kid.column_inline_sizes();
