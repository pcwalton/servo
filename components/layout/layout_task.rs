...
use incremental::{LayoutDamageComputation, ReflowEntireDocument};
use layout_debug;
...
impl LayoutTask {
    ...
    fn handle_reflow<'a>(&'a self,
        ...
        profile(time::LayoutRestyleDamagePropagation,
            ...
            if layout_root.compute_layout_damage().contains(ReflowEntireDocument) {
                layout_root.reflow_entire_document()
            } else {
                println!("NOT reflowing entire document!");
            }
        });
        ...
        if data.goal == ReflowForDisplay {
            ...
            profile(time::LayoutDispListBuildCategory,
                ...
                println!("built display list in layout, len={}", display_list.list.len());
