    ...
    pub fn new(name: String) -> Scope {
        STATE_KEY.with(|ref r| {
            if let Some(ref mut state) = *r.borrow_mut() {
                let flow_trace = to_value(&state.flow_root.read().base()).unwrap();
    ...
    fn drop(&mut self) {
        STATE_KEY.with(|ref r| {
            if let Some(ref mut state) = *r.borrow_mut() {
                ...
                current_scope.post = to_value(&state.flow_root.read().base()).unwrap();
...
pub fn begin_trace(flow_root: FlowRef) {
    ...
    STATE_KEY.with(|ref r| {
        let flow_trace = to_value(&flow_root.read().base()).unwrap();
        let state = State {
...
pub fn end_trace(generation: u32) {
    ...
    root_scope.post = to_value(&thread_state.flow_root.read().base()).unwrap();
