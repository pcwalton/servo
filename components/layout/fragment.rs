...
use inline::{InlineMetrics, LAST_FRAGMENT_OF_ELEMENT, LineBlockMetrics};
...
impl Fragment {
    ...
    fn content_inline_metrics(&self, layout_context: &LayoutContext) -> InlineMetrics {
        ...
        let inline_metrics = match self.specific {
            ...
            SpecificFragmentInfo::Image(_) => {
                ...
                InlineMetrics {
                    block_size_above_baseline: ascent,
                    depth_below_baseline: self.margin.block_end,
                    ascent: ascent,
                }
            }
            SpecificFragmentInfo::ScannedText(ref info) => {
        ...
        //println!("inline metrics of fragment={:?}", inline_metrics);
        ...
        for style in self.inline_styles() {
            ...
            match style.get_box().vertical_align {
                ...
                vertical_align::T::sub => {
                    ...
                    offset += height.scale_by(FONT_SUBSCRIPT_OFFSET_RATIO)
                }
                vertical_align::T::super_ => {
                    let height = minimum_block_size_above_baseline + minimum_depth_below_baseline;
                    offset -= height.scale_by(FONT_SUPERSCRIPT_OFFSET_RATIO)
                }
                ...
                vertical_align::T::text_bottom => {
                    offset = minimum_depth_below_baseline -
                        self.content_inline_metrics(layout_context).depth_below_baseline
                }
                vertical_align::T::top => {
                    if let Some(actual_line_metrics) = actual_line_metrics {
                        offset = self.content_inline_metrics(layout_context).ascent -
                            actual_line_metrics.block_size_above_baseline
                    }
                }
                vertical_align::T::bottom => {
                    if let Some(actual_line_metrics) = actual_line_metrics {
                        offset = actual_line_metrics.depth_below_baseline -
                            self.content_inline_metrics(layout_context).depth_below_baseline
                    }
                }
                ...
                vertical_align::T::LengthOrPercentage(LengthOrPercentage::Percentage(
                        percentage)) => {
                    offset -= (minimum_block_size_above_baseline +
                               minimum_depth_below_baseline).scale_by(percentage)
                }
                vertical_align::T::LengthOrPercentage(LengthOrPercentage::Calc(formula)) => {
                    offset -= (minimum_block_size_above_baseline +
                               minimum_depth_below_baseline).scale_by(formula.percentage()) -
                              formula.length()
                }
            }
        }
    ...
    pub fn aligned_inline_metrics(&self,
        ...
        let vertical_alignment_offset =
            self.vertical_alignment_offset(layout_context,
                                           minimum_block_size_above_baseline,
