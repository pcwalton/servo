...
impl WebGLFramebuffer {
    ...
    pub fn maybe_new_webxr(session: &XRSession, context: &WebGLRenderingContext, size: Size2D<i32, Viewport>) -> Option<(WebXRSwapChainId, DomRoot<Self>)> {
        let (sender, receiver) = webgl_channel().unwrap();
        context.webgl_sender().send_create_webxr_swap_chain(size.to_untyped(), sender);
        let swap_chain_id = receiver.recv().unwrap()?;
        let framebuffer_id = WebGLFramebufferId::Opaque(WebGLOpaqueFramebufferId::WebXR(swap_chain_id));
