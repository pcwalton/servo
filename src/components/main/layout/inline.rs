...
struct LineboxScanner {
    ...
    work_list: @mut RingBuf<@mut RenderBox>,
...
impl LineboxScanner {
    ...
    fn new_height_for_line(&self, new_box: @mut RenderBox) -> Au {
    ...
    fn initial_line_placement(&self,
        ...
        let placement_width = if splitable {
            ...
            first_box.base().position.size.width
        };
        ...
        if line_bounds.size.width > first_box.base().position.size.width {
            debug!("LineboxScanner: case=box fits");
            return (line_bounds, first_box.base().position.size.width);
        }
        ...
        match first_box.split_to_width(line_bounds.size.width, line_is_empty) {
            ...
            SplitDidFit(left, right) => {
                ...
                let actual_box_width = match (left, right) {
                    (Some(l_box), Some(_))  => l_box.base().position.size.width,
                    (Some(l_box), None)     => l_box.base().position.size.width,
                    (None, Some(r_box))     => r_box.base().position.size.width,
    ...
    fn try_append_to_line(&mut self, in_box: @mut RenderBox, flow: &mut InlineFlow) -> bool {
        ...
        let new_width = self.pending_line.bounds.size.width + in_box.base().position.size.width;
    ...
    // unconditional push
    fn push_box_to_line(&mut self, box: @mut RenderBox) {
        ...
        /*do local_data::get_mut(local_linebox_scanner) |maybe_scanner| {
            ...
            match maybe_scanner {
                None => {
                    ...
                    local_data::set(local_linebox_scanner, scanner);
                }
                ...
                for box_i in line.range.eachi() {
                    ...
                    let (top_from_base, bottom_from_base, ascent) = match cur_box.class() {
                        ImageRenderBoxClass => {
                            ...
                            image_box.base.position.translate(&Point2D(Au::new(0), -height));
                    ...
                    let mut parent_text_top = Au::new(0);
